...
export function parseSportterySummary(input: SportterySummaryInput): ParsedContextDomain {
  ...
  if (homeT || awayT) parts.push(`积分形势：主${homeT ?? "无"} 客${awayT ?? "无"}`);
  ...
  if (homeR || awayR) parts.push(`近期状态：主${homeR ?? "无"} 客${awayR ?? "无"}`);
  ...
  if (homeI || awayI) parts.push(`伤停影响：主[${homeI || "无"}] 客[${awayI || "无"}]`);
