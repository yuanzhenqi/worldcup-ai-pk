...
const contextRules = [
  ...
  "若 prediction_context 包含体彩数据（sporttery，含官方竞彩胜平负/让球赔率、历史交锋、积分榜、近期战绩、特征胜率、伤停明细），让球盘口反映官方市场观点可作背景参考但不得直接决定预测；积分榜反映小组出线形势；伤停明细可直接用于评估阵容影响与关键球员缺阵风险；官方赔率不得作为胜平负预测的权重。",
  "输出必须包含结构化字段和中文摘要。"
].join("\n");
